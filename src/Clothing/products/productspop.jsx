...
const Productpop = ({ product, onClose, addToCart }) => {
    ...
    <div className="fixed inset-0 shadow-2xl flex justify-center items-center z-50 ">
      <div className="bg-gray-300 p-6 rounded-md w-full max-w-4xl relative flex">
        ...
        <div className="w-1/2 p-4">
          ...
          <div className="mt-4">
            <span className="text-green-700 text-2xl font-bold">{product.price}</span>
          </div>
          <button
            onClick={() => addToCart(product)}
            className="mt-6 bg-green-700 hover:bg-green-800 text-white px-6 py-3 rounded-md"
          >
          ...
          <div className="flex gap-2 items-center mt-3">
            sellers <span className="text-green-700 underline">{product.seller}</span>
          </div>
        </div>
      </div>
    </div>
